...
router.get("/username/:username/thoughts", authOptional, wrapAsync(getUserThoughts));
...
// 4️⃣ Follow routes (ID based) - Protected by strict Anti-Botnet Rate Limits
router.post("/:id/follow", isLoggedIn, followRateLimiters, wrapAsync(followUser));
router.post("/:id/unfollow", isLoggedIn, unfollowRateLimiters, wrapAsync(unfollowUser));
